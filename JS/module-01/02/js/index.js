...
//         вывести сообщение 'Вы купили [число] дроидов, на счету осталось 
//         [число] кредитов.'.
...
let totalBuy = prompt('Сколько дроидов Вы покупаете?');
if (totalBuy === null) {
    alert('Отменено пользователем!');
} else {
    const toNumber = Number(totalBuy);
    if (Number.isNaN(toNumber)) {
        alert('Ошибка ввода!');
    } else {
        totalPrice = pricePerDroid * toNumber;
        solvency = credits - totalPrice;

        if (solvency < 0) {
            alert('Недостаточно средств на счету!');
        } else {
            alert(`Вы купили ${totalBuy} дроидов, на счету осталось ${solvency} кредитов`)
        }
    }
}
